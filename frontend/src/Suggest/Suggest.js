      ...
      <div className={'main-right-fixed'}>
        <div className={'myProfile'}>
          ...
          <div className={'myProfile-text'}>
            {user ? (
              <Link to={'user'} className={'my-id'}>
                {user.username}
              </Link>
            ) : (
              <span className={'my-id'}></span>
            )}{' '}
            {/*받아온 user가 있을 때는 username 표시하고 없으면 공백 처리*/}
          </div>
          <span className={'follow'}>전환</span>
        </div>
        <div className={'suggestion'}>
          <span className={'left'}>회원님을 위한 추천</span>
          <Link to={'/instagram/people'} className={'right'}>
            모두 보기
          </Link>
        </div>
        ...
        <div className={'else-nav'}>
          ...
          <span>
            <a
              href={'https://help.instagram.com/581066165581870/?locale=ko_KR'}
              target={'_blank'}
              rel="noopener noreferrer"
            >
              약관
        ...
        <div className={'else-text'}>© 2022 INSTAGRAM FROM META</div>
      </div>
    </div>
  );
};
